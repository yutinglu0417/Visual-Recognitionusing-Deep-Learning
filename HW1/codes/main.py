...
def con_loss(features, labels):
    ...
    loss = (
        (pos_cos_matrix * pos_label_matrix).sum() +
...
def plot_acc_loss(
    ...
    plt.figure(figsize=(8, 6))
    plt.plot(
        epochs, train_loss_list, label='Train Loss',
        marker='o', color='r'
    )
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
...
def train(
    ...
    for images, labels in tqdm(dataloader, desc='train', leave=False):
        ...
        loss, outputs = train_one_batch(
            model, criterion, optimizer, images, labels
        )
        total_loss += loss
...
def main():
    train_dl, valid_dl, test_dl, model, criterion, optimizer, scheduler = \
        setup()
    if (MODE == "train"):
        best_acc = 0
        train_acc_list = []
        ...
        for epoch_num in tqdm(range(1, NUM_EPOCH + 1)):
            loss, train_acc = train(
                model, train_dl, criterion, optimizer, scheduler
            )
            ...
            print(
                f"epoch {epoch_num}: train loss: {loss:.4f}, "
                f"train accuracy: {train_acc:.4f}, "
                f"current accuracy: {acc:.4f}, best accuracy: {best_acc:.4f}"
            )
