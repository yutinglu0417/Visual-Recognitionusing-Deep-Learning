...
class PromptGenBlock_new(nn.Module):
    def __init__(self, prompt_dim=128, prompt_len=5, prompt_size=96, lin_dim=192):
        super(PromptGenBlock_new, self).__init__()
        
        # prompt pool: learnable tensor of shape [1, L, C, H, W]
        self.prompt_param = nn.Parameter(
            torch.rand(1, prompt_len, prompt_dim, prompt_size, prompt_size)
        )
        
        # 使用 mean, std, max → 所以是 lin_dim * 3
        self.linear_layer = nn.Linear(lin_dim * 3, prompt_len)
    ...
    def forward(self, x):
        ...
        mean = x.mean(dim=(-2, -1))
        std = x.std(dim=(-2, -1))
        maxv = x.amax(dim=(-2, -1))  # max over H, W

        # 拼接成 feature vector，維度變成 [B, C * 3]
        feat = torch.cat([mean, std, maxv], dim=-1)

        # 通過線性層 → softmax 得到每個 prompt 的權重
        prompt_weights = F.softmax(self.linear_layer(feat), dim=1)  # [B, L]

        # 取出 prompt_param 並重複 B 批次
        prompt_pool = self.prompt_param.expand(B, -1, -1, -1, -1)  # [B, L, C, H, W]

        # 加權組合
        weights = prompt_weights.view(B, prompt_weights.shape[1], 1, 1, 1)  # [B, L, 1, 1, 1]
        prompt = torch.sum(weights * prompt_pool, dim=1)  # [B, C, H, W]
...
class ConvFormerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type):
        ...
        self.attn = TransformerBlock(dim=dim, num_heads=num_heads,
    ...
    def forward(self, x):
        # conv branch
        x_conv = self.conv(x)
        # transformer expects (B, N, C), convert and normalize
        B, C, H, W = x.shape
        # x_flat = x.permute(0, 2, 3, 1).reshape(B, H*W, C)
        x_trans = self.attn(x)
        x_trans = x_trans.view(B, H, W, C).permute(0, 3, 1, 2)
        return x_conv + x_trans  # residual fusion
...
class PromptIR(nn.Module):
    def __init__(self, 
        ...
        if self.decoder:
            self.prompt1 = PromptGenBlock_new(prompt_dim=64,prompt_len=5,prompt_size = 64,lin_dim = 96)
            self.prompt2 = PromptGenBlock_new(prompt_dim=128,prompt_len=5,prompt_size = 32,lin_dim = 192)
            self.prompt3 = PromptGenBlock_new(prompt_dim=320,prompt_len=5,prompt_size = 16,lin_dim = 384)
        ...
        self.encoder_level1 = self.encoder_level1 = nn.Sequential(*[
            ConvFormerBlock(dim, heads[0], ffn_expansion_factor, bias, LayerNorm_type)
